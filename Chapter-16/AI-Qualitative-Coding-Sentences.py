...
# %%
# Break postings into sentences
################################################################################
xpostings = pd.DataFrame(columns=['pid', 'sid', 'text']) # Expanded postings
for postnum, post in enumerate(df.text):
    sentences = nltk.sent_tokenize(post)
    for sentencenum, sentence in enumerate(sentences):
        row = {'pid'  : postnum,
               'sid'  : sentencenum,
               'text' : sentence}
        dfrow = pd.DataFrame([row]) # since scalar row need []
        xpostings = pd.concat([xpostings, dfrow])

postings = list(xpostings.text)
# %%
...
dft = pd.DataFrame(xpostings)
dft = xpostings.reset_index(drop = True)
dff = pd.concat([dft, dfe], axis=1)
# %%
out_file = 'IAM42-Analysis-Sentence.csv' # Change this
